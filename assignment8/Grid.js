class Grid {
  ...
  constructor(_w, _h) {
    this.gridWidth = _w;
    this.gridHeight = _h;
    this.noteSize = 15; // Changed the size of the notes to make it nicer
    this.notePos = [];
    this.noteState = [];
  ...
  // ============ I wrote this ================
  handleSounds() {
    for (var i = 0; i < this.noteState.length; i++) {
      for (var j = 0; j < this.noteState[i].length; j++) {
        if (this.noteState[i][j] > 0) {
          if ((i >= 0 && i <= 15) && !noteDo.isPlaying()) {
            noteDo.play();
          } else if ((i >= 16 && i <= 20) && !noteRe.isPlaying()) {
            noteRe.play();
          } else if ((i >= 21 && i <= 25) && !noteMi.isPlaying()) {
            noteMi.play();
          } else if ((i >= 26 && i <= 30) && !noteFa.isPlaying()) {
            noteFa.play();
          } else if ((i >= 31 && i <= 35) && !noteSol.isPlaying()) {
            noteSol.play();
          } else if ((i >= 36 && i <= 45) && !noteLa.isPlaying()) {
            noteLa.play();
          }
        }
      }
